...
udp_forward::udp_forward(asio::io_service& io_service,
		...
		const asio::ip::udp::endpoint& remote_endpoint, const string& key, bool debug) :
	...
	boost::asio::socket_base::receive_buffer_size receive_buffer_size_option(65536);
...
void udp_forward::handle_server_receive(const boost::system::error_code& ec_receive,
		std::size_t bytes_transferred) {
	boost::system::error_code ec;
	std::shared_ptr<connection> pconn;
	if (ec_receive) {
		cerr << __FUNCTION__ << ":" << __LINE__ << ": " << ec_receive.message() << endl;
		goto end_of_handle_server_receive;
	}
	if (debug) {
	...
	pconn.reset();
	for (auto ipconn2 = connections.begin(); ipconn2 != connections.end(); ++ipconn2) {
	...
	if (!pconn) {
		pconn = std::shared_ptr<connection>(
				new connection(sender_endpoint,
						...
						posix_time::microsec_clock::universal_time()));
		pconn->client_socket.connect(remote_endpoint, ec);
		if (ec) {
...
end_of_handle_server_receive:;
	start_server_receive();
}

void udp_forward::handle_client_receive(std::shared_ptr<connection> pconn,
		const boost::system::error_code& ec_receive, std::size_t bytes_transferred) {
	if (ec_receive) {
		cerr << __FUNCTION__ << ":" << __LINE__ << ": " << ec_receive.message() << endl;
		boost::system::error_code ec;
		pconn->client_socket.close(ec);
...
void udp_forward::handle_clean_timer(const boost::system::error_code& ec_timer) {
	if (ec_timer) {
		cerr << __FUNCTION__ << ":" << __LINE__ << ": " << ec_timer.message() << endl;
		return;
	}
	...
	for (auto iipconn = to_be_removed.begin(); iipconn != to_be_removed.end(); ++iipconn) {
